$(document).ready(function() {
	...
	function isReady() {
		...
		var mode = $("#li-signup").attr("class") == "active" ? "SIGNUP" : "SIGNIN";
		var isEmailCorr = false;
		var isPwCorr = false;
		var isPwFocus = $("#login-password").is(":focus");	//TODO: how to avoid unnecessary validation ?
		// Email checking
		$(".err-msg").remove();								// initialize .err-msg 
		$("#div-email").attr("class", "form-group");		// initialize #div-email
		if (email == "") {
			$("#div-email").addClass("has-error");
			$("#div-email").append("<p id=\"p-email\" class=\"text-danger err-msg\">Email vide</p>");
		} else if (!reg.test(email)) {
			$("#div-email").addClass("has-error");
			$("#div-email").append("<p id=\"p-email\" class=\"text-danger err-msg\">Format incorrect</p>");
		} else {
			isEmailCorr = true;
		}
		// Password initializing
		$("#div-password").attr("class", "form-group");		// initialize #div-password
		$("#div-password2").attr("class", "form-group");	// initialize #div-password2
		if(mode == "SIGNIN")
			$("#div-password2").addClass("hidden");			// initialize #div-password2
		// Password checking
		if (isEmailCorr && !isPwFocus) {
			if (pw == "") {
				$("#div-password").addClass("has-error");
				$("#div-password").append("<p id=\"p-password\" class=\"text-danger err-msg\">Mdp vide</p>");
			} else if(mode == "SIGNUP" && pw2 == "") {
				$("#div-password2").addClass("has-error");
				$("#div-password2").append("<p id=\"p-password2\" class=\"text-danger err-msg\">Mdp vide</p>");
			} else if(mode == "SIGNUP" && pw != pw2) {
				$("#div-password").addClass("has-error");
				$("#div-password2").addClass("has-error");
				$("#div-password2").append("<p id=\"p-password2\" class=\"text-danger err-msg\">Mdp incorrect</p>");
			} else {
				isPwCorr = true;
			}
		} 
