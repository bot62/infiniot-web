...
$(document).ready(function() {
  ...
  $('#highcharts-container').highcharts({
    ...
    yAxis:
        {minPadding: 0.2, maxPadding: 0.2, title: {text: 'Value', margin: 80}},
...
function requestData() {
  $.ajax({
    ...
    method: 'POST',
    data: {key: '72cec25cbbd386c3b68f9b73f480057ef55d09b8'},
...
addSdata =
  ...
  if (chart.series.length === 0) {
    $.each(sensors, function(i, sensor) {
      chart.addSeries({name: sensor.sid, data: []});
    });
  }
...
trClick = function(trTarget) {
  ...
  $(trTarget).each(function() {
    ...
    $(this).not(':has(th)').click(function() {
      ...
      chart.setTitle({text: $(this).attr('id')});
...
$(document).ready(function() {
  if (mq.matches) {
    alert('ok')
    $('#highcharts-container').highcharts.yAxis.title.attr({enabled: false});
  }
});
